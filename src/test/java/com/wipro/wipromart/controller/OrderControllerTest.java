...
public class OrderControllerTest {
	...
	void testAddOrder() throws Exception {
		...
		mockMvc.perform(post("/order/save").contentType(MediaType.APPLICATION_JSON)
	...
	/*
	 * @Test void testFetchOrder() throws Exception { Customer customer = new
	 * Customer(); customer.setCustomerId(1L);
	 * 
	 * OrderItem orderItem = new OrderItem(); orderItem.setQuantity(2);
	 * 
	 * orderItem.setProductId(101L); orderItem.setItemTotal(200.0);
	 * 
	 * Order order = new Order(); order.setOrderId(1); order.setCustomer(customer);
	 * order.setOrderItems(Arrays.asList(orderItem)); order.setOrderAmount(200.0);
	 * order.setOrderStatus("Success");
	 * 
	 * when(orderService.getOrderById(1)).thenReturn(order);
	 * 
	 * mockMvc.perform(get("/order/get/1")).andExpect(status().isOk())
	 * .andExpect(jsonPath("$.orderId").value(order.getOrderId()))
	 * .andExpect(jsonPath("$.orderAmount").value(order.getOrderAmount()))
	 * .andExpect(jsonPath("$.orderStatus").value(order.getOrderStatus()));
	 * 
	...
	/*
	 * @Test void testFetchAllOrders() throws Exception { Customer customer1 = new
	 * Customer(); customer1.setCustomerId(1L);
	 * 
	 * OrderItem orderItem1 = new OrderItem(); orderItem1.setQuantity(2);;
	 * orderItem1.setProductId(101L); orderItem1.setItemTotal(200.0);
	 * 
	 * Order order1 = new Order(); order1.setOrderId(1);
	 * order1.setCustomer(customer1);
	 * order1.setOrderItems(Arrays.asList(orderItem1));
	 * order1.setOrderAmount(200.0); order1.setOrderStatus("Success");
	 * 
	 * Customer customer2 = new Customer(); customer2.setCustomerId(2L);
	 * 
	 * OrderItem orderItem2 = new OrderItem(); orderItem2.setQuantity(3);
	 * orderItem2.setProductId(102L); orderItem2.setItemTotal(300.0);
	 * 
	 * Order order2 = new Order(); order2.setOrderId(2);
	 * order2.setCustomer(customer2);
	 * order2.setOrderItems(Arrays.asList(orderItem2));
	 * order2.setOrderAmount(300.0); order2.setOrderStatus("Success");
	 * 
	 * when(orderService.getAllOrders()).thenReturn(Arrays.asList(order1, order2));
	 * 
	 * mockMvc.perform(get("/order/get/all")) .andExpect(status().isOk())
